...
runTest( int argc, char** argv) 
{
	...
	#pragma acc data copy(input_itemsets[:max_rows * max_cols],referrence[:max_rows * max_cols])
	{
	//Compute top-left matrix 
	printf("Processing top-left matrix\n");
	
    for( int i = 0 ; i < max_cols-2 ; i++){
    	#pragma acc parallel loop present(input_itemsets[:max_rows * max_cols],referrence[:max_rows * max_cols])
		for( idx = 0 ; idx <= i ; idx++){
