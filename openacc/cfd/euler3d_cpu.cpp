...
{
#pragma acc kernels loop independent present_or_copyin(src[0:N]) present_or_create(dst[0:N])
	for(int i = 0; i < N; i++)
	{
		dst[i] = src[i];
	}
...
{
#pragma acc kernels loop independent collapse(2) copyin(ff_variable[0:NVAR]) copyout(variables[0:nelr*NVAR])
...
{
#pragma acc kernels loop independent present(variables[0:nelr*NVAR], areas[0:nelr], step_factors[0:nelr])
...
#pragma acc kernels present(variables[0:nelr*NVAR], normals[0:nelr*3], fluxes[0:nelr*NVAR], elements_surrounding_elements[0:nelr*4], ff_variable[0:NVAR], ff_flux_contribution_momentum_x, ff_flux_contribution_momentum_y, ff_flux_contribution_momentum_z, ff_flux_contribution_density_energy)
    for(int blk = 0; blk < nelr/block_length; ++blk)
    {
        int b_start = blk*block_length;
        int b_end = (blk+1)*block_length > nelr ? nelr : (blk+1)*block_length;
...
{
#pragma acc kernels present(old_variables[0:(nelr*NVAR)], variables[0:(nelr*NVAR)], step_factors[0:nelr], fluxes[0:(nelr*NVAR)])
    for(int blk = 0; blk < nelr/block_length; ++blk)
    {
        int b_start = blk*block_length;
        int b_end = (blk+1)*block_length > nelr ? nelr : (blk+1)*block_length;
...
#pragma acc data create(old_variables[0:(nelr*NVAR)]) copyin(nelr, areas[0:nelr], step_factors[0:nelr], elements_surrounding_elements[0:(nelr*NNB)], normals[0:(NDIM*NNB*nelr)], fluxes[0:(nelr*NVAR)], ff_variable[0:NVAR]) \
	copyin(ff_flux_contribution_momentum_x, ff_flux_contribution_momentum_y, ff_flux_contribution_momentum_z, ff_flux_contribution_density_energy) copy(variables[0:(nelr*NVAR)])
{
	// Begin iterations
